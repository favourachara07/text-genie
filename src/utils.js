...
export const detectLanguage = async (detector, text, setDetectedLanguage) => {
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      const detectedLanguageCode = topResult.detectedLanguage;
      const detectedLanguageName = languageNames[detectedLanguageCode] || detectedLanguageCode;
      setDetectedLanguage(
        `I'm ${Math.round(topResult.confidence * 100)}% sure that the language is ${detectedLanguageName}.`
      );
    } else {
